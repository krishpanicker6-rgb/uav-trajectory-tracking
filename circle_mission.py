...
class CircleMission(Node):
    ...
    def __init__(self):
        ...
        self.center_x = 0.0
        self.center_y = 0.0
        self.circle_duration = 25.0
        self.takeoff_hold_time = 5.0

        self.state = "TAKEOFF"
        self.landing_sent = False

        self.csv_file = open("circle_data.csv", "w", newline="")
    ...
    def loop(self):
        ...
        if self.state == "TAKEOFF":
            ...
            circle_elapsed = time.time() - self.circle_start_time

            theta = 2.0 * math.pi * (circle_elapsed / self.circle_duration)

            desired_x = self.center_x + self.radius * math.cos(theta)
            desired_y = self.center_y + self.radius * math.sin(theta)
            desired_z = self.altitude
            ...
            print(f"CIRCLE | theta {theta:.2f} | error {error:.3f}")

            if circle_elapsed >= self.circle_duration:
                self.state = "RETURN"
