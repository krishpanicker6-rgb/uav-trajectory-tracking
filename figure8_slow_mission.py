...
class Figure8SlowMission(Node):

    def __init__(self):
        super().__init__('figure8_slow_mission')
        ...
        self.altitude = -2.0
        self.amplitude = 2.0
        self.figure8_duration = 60.0
        self.takeoff_hold_time = 5.0

        self.state = "TAKEOFF"
        self.landing_sent = False

        self.csv_file = open("figure8_slow_data.csv", "w", newline="")
        ...
        self.get_logger().info("Slow figure-8 mission started")
    ...
    def loop(self):
        ...
        if self.state == "TAKEOFF":
            ...
            if elapsed > self.takeoff_hold_time and error < 0.4:
                ...
                self.get_logger().info("Starting SLOW figure-8 trajectory")
            ...
            self.log_data(desired_x, desired_y, desired_z, error)
            print(f"SLOW FIGURE8 | theta {theta:.2f} | error {error:.3f}")

            if t >= self.figure8_duration:
                self.state = "RETURN"
                self.get_logger().info("Slow figure-8 complete, returning home")
...
def main():
    ...
    node = Figure8SlowMission()

    try:
        rclpy.spin(node)
    except KeyboardInterrupt:
        print("Slow figure-8 mission stopped")
